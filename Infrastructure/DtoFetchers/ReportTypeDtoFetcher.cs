...
{    
    /// <summary>
    /// Фетчер DTO типа отчетов.
    /// </summary>
    public class ReportTypeDtoFetcher : BaseDtoFetcher<ReportTypeDao, ReportTypeDto>
    {
        static ReportTypeDtoFetcher()
        {
            CreateMapForIndex();
            CreateMapForList();
            CreateMapForCard();
        ...
        private static void CreateMapForList()
        {
            ...
            map.Map(d => d.Id, e => e.Id)
                ...
                .Map(d => d.GroupId, e => e.GroupId)
                .Map(d => d.GroupName, e => e.Group.Name)
                ;
        ...
        private static void MapSpecificForList(IFetchDtoMap<ReportTypeDao, ReportTypeDto> map)
        {
        ...
        private static void MapSpecificForCard(IFetchDtoMap<ReportTypeDao, ReportTypeDto> map)
        {
            //map.Map(d => d.Id, e => e.C_Tree)
            //.Map(d => d.ParentTreeId, e => e.C_Parent_Tree);
        }

        public ReportTypeDtoFetcher(IRepository repository)
            : base(repository)
        {
        }
    }
