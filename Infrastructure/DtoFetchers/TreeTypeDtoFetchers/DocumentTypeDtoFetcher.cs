...
{
    /// <summary>
    /// Фетчер DTO типа документов.
    /// </summary>
    public class DocumentTypeDtoFetcher : BaseDtoFetcher<DocumentTypeDao, DocumentTypeDto>
    {
        static DocumentTypeDtoFetcher()
        {
            CreateMapForIndex();
            CreateMapForList();
            CreateMapForCard();
        ...
        private static void MapSpecificForList(IFetchDtoMap<DocumentTypeDao, DocumentTypeDto> map)
        {
        ...
        private static void MapSpecificForCard(IFetchDtoMap<DocumentTypeDao, DocumentTypeDto> map)
        {
            //map.Map(d => d.Id, e => e.C_Tree)
            //.Map(d => d.ParentTreeId, e => e.C_Parent_Tree);
        }

        public DocumentTypeDtoFetcher(IRepository repository)
            : base(repository)
        {
        }
    }
